...
/**
 * Security configuration for the QuantBackEngine API.
 *
 * Authentication is disabled — the backend is secured by network isolation:
 * - It is only reachable inside the Docker network.
 * - The Next.js frontend proxies all /api requests to the backend.
 * - External users access port 3000 (frontend) only; port 8080 is not exposed
 * publicly.
 *
